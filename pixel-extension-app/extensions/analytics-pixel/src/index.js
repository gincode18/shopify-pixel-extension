...
function extractProducts(eventData, initData = null) {
  ...
  if (eventData?.cartLine) {
    products.push({
      ...
      price: eventData.cartLine.merchandise?.price
        ? parseFloat(eventData.cartLine.merchandise.price.amount)
        : undefined,
  ...
  if (eventData?.variant) {
    products.push({
      ...
      price: eventData.variant.price
        ? parseFloat(eventData.variant.price.amount)
        : undefined,
...
function getCurrency(eventData) {
  return (
    eventData.data?.checkout?.currencyCode ||
...
register(async ({ analytics, browser, settings, init }) => {
  ...
  const webhookUrl =
    settings.webhookUrl ||
    "https://poc-shopify-wheat.vercel.app/webhook/shopify-events";
  ...
  try {
    ...
    if (!muid) {
      ...
      await browser.cookie.set(
        `muid=${muid}; expires=${expiryDate.toUTCString()}; path=/`
      );
  ...
  console.log(
    "Pixel is running, which means consent has been granted or is not required"
  );
  console.log(
    'If you see this message in testing but the pixel still shows "awaiting consent", refresh the page'
  );
  ...
  analytics.subscribe("all_events", async (event) => {
    ...
    try {
      ...
      const pageUrl =
        ...
        event.context?.document?.url ||
        "";

      // Extract email and phone from checkout data
      const customerEmail =
        event.data?.checkout?.email ||
        event.checkout?.email ||
        event.context?.customer?.email ||
        allCookies.email ||
        undefined;

      const customerPhone =
        event.data?.checkout?.phone ||
        event.checkout?.phone ||
        event.context?.customer?.phone ||
        allCookies.phone ||
        undefined;
      ...
      switch (markTagEventType) {
        ...
        case "Purchase":
          ...
          const purchaseShippingAddress =
            event.data?.checkout?.shippingAddress ||
            event.checkout?.shippingAddress ||
            event.shippingAddress;
          const purchaseShippingLine =
            event.data?.checkout?.shippingLine ||
            event.checkout?.shippingLine ||
            event.shippingLine;

          // Extract customer email and phone specifically for Purchase events
          const purchaseEmail =
            event.data?.checkout?.email ||
            event.checkout?.email ||
            customerEmail;
          ...
          eventPayload = {
            ...
            products: products,
            transaction_id:
              event.checkout?.token ||
              event.data?.checkout?.token ||
              event.id?.toString(),
            tax: event.data?.checkout?.totalTax?.amount
              ? parseFloat(event.data.checkout.totalTax.amount)
              : event.checkout?.totalTax?.amount
              ? parseFloat(event.checkout.totalTax.amount)
              : undefined,
            shipping_cost: purchaseShippingLine?.price?.amount
              ? parseFloat(purchaseShippingLine.price.amount)
              : undefined,
            // Ensure email and phone are included in the Purchase event payload
            email: purchaseEmail,
            phone: purchasePhone,
            ...
            loc:
              purchaseShippingAddress?.countryCode ||
              event.checkout?.shippingAddress?.countryCode ||
        ...
        case "InitiateCheckout":
          // Extract customer email and phone specifically for BeginCheckout events
          const checkoutEmail =
            event.data?.checkout?.email ||
            event.checkout?.email ||
            customerEmail;
          ...
          eventPayload = {
            ...
            value: value,
            products: products,
            tax: event.checkout?.totalTax?.amount
              ? parseFloat(event.checkout.totalTax.amount)
              : undefined,
            shipping_cost: event.checkout?.shippingLine?.price?.amount
              ? parseFloat(event.checkout.shippingLine.price.amount)
              : undefined,
            // Ensure email and phone are included in the BeginCheckout event payload
            email: checkoutEmail,
            phone: checkoutPhone,
            // Extract shipping address information
            shipping_address: {
              country:
                event.checkout?.shippingAddress?.country ||
              ...
              country_code:
                ...
                undefined,
              province:
                event.checkout?.shippingAddress?.province ||
                event.shippingAddress?.province ||
                undefined,
              province_code:
                event.checkout?.shippingAddress?.provinceCode ||
                event.shippingAddress?.provinceCode ||
                undefined,
              city:
                event.checkout?.shippingAddress?.city ||
                event.shippingAddress?.city ||
                undefined,
              zip:
                event.checkout?.shippingAddress?.zip ||
                event.shippingAddress?.zip ||
                undefined,
            },
            loc:
              event.checkout?.shippingAddress?.countryCode ||
        ...
        case "AddPaymentInfo":
          // Extract customer email and phone specifically for AddPaymentInfo events
          const paymentEmail =
            event.data?.checkout?.email ||
            event.checkout?.email ||
            customerEmail;
          ...
          eventPayload = {
            ...
            payment_type:
              ...
              event.checkout?.paymentMethod ||
              undefined,
            value: value.toString(),
            currency: currency,
            products: products,
            // Ensure email and phone are included in the AddPaymentInfo event payload
            email: paymentEmail,
            phone: paymentPhone,
          };
          break;
        ...
        case "AddShippingInfo":
          ...
          // Extract customer email and phone specifically for AddShippingInfo events
          const shippingEmail =
            event.data?.checkout?.email ||
            event.checkout?.email ||
            customerEmail;
          ...
          eventPayload = {
            ...eventPayload,
            shipping_tier:
              shippingLine?.title || event.shippingRate?.title || undefined,
            value: value.toString(),
            currency: currency,
            products: products,
            // Ensure email and phone are included in the AddShippingInfo event payload
            email: shippingEmail,
            phone: shippingPhone,
            // Extract shipping address information
            shipping_address: {
      ...
      if (markTagEventType === "AddShippingInfo") {
        ...
        console.log("  - event.checkout?.email:", event.checkout?.email);
        console.log(
          "  - event.data?.checkout?.phone:",
          event.data?.checkout?.phone
        );
        console.log("  - event.checkout?.phone:", event.checkout?.phone);
      }
      ...
      if (!response.ok) {
        console.error(
          "❌ Failed to send MarkTag event to webhook. Status:",
          response.status
        );
        console.error(
          "🔴 Event type:",
        ...
        console.log(
          "✅ MarkTag event successfully sent:",
          markTagEventType,
          "(from",
          event.name + ")"
        );
